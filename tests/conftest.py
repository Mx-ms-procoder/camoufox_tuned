...
def event_loop() -> Generator[asyncio.AbstractEventLoop, None, None]:
    ...
    # 'MainThread'") -- errors that read like catastrophic browser failures.
...
def _to_camel_case_keys(options: Dict) -> Dict:
    ...
    validates types strictly, so a `channel: None` from an unused
    --browser-channel aborts the server with "channel: expected string, got
    object".
    """
...
class RemoteServer:
    def __init__(self, browser_name: str, launch_server_options: Dict, tmpfile: Path) -> None:
        ...
        # doesn't exist", printed no endpoint, and every connect test then died
        # on an empty ws_endpoint with a misleading port error.
