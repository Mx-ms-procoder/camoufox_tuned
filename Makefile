...
# ./mach clobber FIRST: `git clean -fdx` deletes mach itself (it is untracked in
# the extracted tree), so the old order left clobber to fail with "mach: not
# found" and the objdir was never actually cleared.
clean:
...
build: unbusy
	...
	cd $(cf_source_dir) && ./mach build $(MACH_BUILD_ARGS) $(_ARGS)
...
package-windows:
	python3 scripts/package.py windows \
		--includes \
			...
			settings/camoucfg.jvv \
			settings/properties.json \
			~/.mozbuild/vs/VC/Redist/MSVC/14.*/$(vcredist_arch)/Microsoft.VC*.CRT/*.dll \
		--version $(version) \
		--release $(release) \
		--arch $(arch) \
		--fonts macos linux
