...
static inline void ApplyCanvasNoise(uint8_t* aData, int32_t aStride,
  ...
  for (int32_t y = 0; y < aHeight; ++y) {
    ...
    for (int32_t x = 0; x < aWidth; ++x) {
      ...
      for (int c = 0; c < 3; ++c) {
        // Symmetric delta in {-1, 0, 1}; no +2 brightness bias.
        int delta = static_cast<int>((h >> (8 + c * 2)) % 3ULL) - 1;
...
// Apply to exactly the requested rectangle. The caller supplies the actual
// pack stride/offset and source coordinates; padding and suffix bytes are inert.
// Invalid layouts are rejected before the first write. The optional defaults
// model WebGL's PACK_ALIGNMENT=4 for existing tightly packed helper callers.
static inline void ApplyByteReadbackNoise(uint8_t* aData, size_t aSizeBytes,
                                          ...
                                          size_t aStride = 0, size_t aOffset = 0,
                                          uint32_t aSourceX = 0,
                                          uint32_t aSourceY = 0) {
  if (!aData || !aSeed || aWidth <= 0 || aHeight <= 0 ||
      !aElementsPerPixel || aElementsPerPixel > 4 || aOffset > aSizeBytes) return;
  const size_t width = static_cast<size_t>(aWidth);
  if (width > std::numeric_limits<size_t>::max() / aElementsPerPixel) return;
  const size_t rowBytes = width * aElementsPerPixel;
  if (!aStride) {
    if (rowBytes > std::numeric_limits<size_t>::max() - 3) return;
    aStride = (rowBytes + 3) & ~size_t(3);
  }
  const size_t available = aSizeBytes - aOffset;
  if (aStride < rowBytes || rowBytes > available ||
      static_cast<size_t>(aHeight - 1) > (available - rowBytes) / aStride) return;
  const uint32_t alpha = aHasAlpha ? aElementsPerPixel - 1 : aElementsPerPixel;
  for (uint32_t y = 0; y < static_cast<uint32_t>(aHeight); ++y) {
    auto* row = aData + aOffset + static_cast<size_t>(y) * aStride;
    for (uint32_t x = 0; x < static_cast<uint32_t>(aWidth); ++x) {
      auto* px = row + static_cast<size_t>(x) * aElementsPerPixel;
      uint64_t h = aSeed ^ (uint64_t(aSourceX + x) << 32) ^ (aSourceY + y);
      for (uint32_t c = 0; c < aElementsPerPixel; ++c) {
        h ^= static_cast<uint64_t>(px[c]) << (c * 8);
      }
      h = CanvasMix64(h);
      if ((h & 7) != 0) continue;
      for (uint32_t c = 0; c < aElementsPerPixel; ++c) {
        if (c == alpha) continue;
        const int delta = static_cast<int>((h >> (8 + c * 2)) % 3) - 1;
