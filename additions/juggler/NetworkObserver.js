...
// request came from an automated browser -- and page.route() is enough to trigger it. Put the two
// headers back in the order necko fills them in: the defaults, "connection", whatever the channel
// was set up with, "cookie", and finally everything added from http-on-modify-request onwards.
// Order on the wire follows nsHttpHeaderArray, and setting a header that is already present
// updates it in place, so moving one means deleting it and adding it back.
